...
{
    public partial class frmProgramProperties : Form
    {
        private MainForm m_pForm;
        private REngine m_pEngine;

        public frmProgramProperties()
        {
            InitializeComponent();
            m_pForm = System.Windows.Forms.Application.OpenForms["MainForm"] as MainForm;
            m_pEngine = m_pForm.pEngine;
            nudFeatureCnt.Maximum = m_pForm.intWarningCount;
            nudFeatureCnt.Value = m_pForm.intWarningCount;
            ...
            //int intCnt = m_pForm.blnsInstalledPcks.Length;

            //for (int i = 0; i < intCnt; i++)
            //{
            //    ListViewItem lvi = new ListViewItem(pPckNames.RequiredPackageNames[i]);
            //    lvi.SubItems.Add(m_pForm.blnsInstalledPcks[i].ToString());
        ...
        private void txtREnviron_DoubleClick(object sender, EventArgs e)
        {
            frmREnvironment pfrmREnviron = new frmREnvironment();
            pfrmREnviron.Show();
        }
