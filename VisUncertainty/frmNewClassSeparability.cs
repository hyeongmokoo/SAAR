...
{
    public partial class frmNewClassSeparbility : Form
    {
        ...
        public double[] m_dblpValue;
        private double[] m_arrSortedProV;
        private int[] m_intResultIdx; 
        ...
        public frmNewClassSeparbility()
        {
            try
            {
                ...
                AddMultiRamps(listFormcolor, m_intLstCounts, RedToBlueColorRamps());
        ...
        private void btnApply_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //clsClassSeperabilityMembers pCSMembers = new clsClassSeperabilityMembers();
                //pCSMembers.ClassBrks = m_cb;
                //pCSMembers.Colors = m_arrColors;
                //pCSMembers.Serabability = m_dblpValue;
                //pCSMembers.ClassBrksCnt = m_intGCBreakeCount;
                ...
                //pClassSeparabilityRenderer.pCSMembers = pCSMembers;
                pClassSeparabilityRenderer.intClassBrksCnt = m_intGCBreakeCount;
                pClassSeparabilityRenderer.arrClassBrks = m_cb;
                pClassSeparabilityRenderer.arrColors = m_arrColors;
                pClassSeparabilityRenderer.arrSeparability = m_dblpValue;
                //pClassSeparabilityRenderer.arrSepLineColors = RedToBlueColorRamps();
                pClassSeparabilityRenderer.dblLineWidth = dblGCOutlineSize;
                pClassSeparabilityRenderer.intRoundingDigits = m_intRounding;
        ...
        //            m_cb = ClassSeparabilityMeasure(m_intGCBreakeCount, m_intNofFeatures, pSortedCursor, intValueFldIdx, intUncernfldIdx);
        ...
        public void DrawSymbolinChartwithCb(double[] Cs, int intGCBreakeCount)
        {
            try
            {
                ...
                int intSepLineIdx = 0;
                System.Drawing.Color pLineColor = new Color();

                for (int j = 1; j < m_cb.Length - 1; j++)
                {
                    intSepLineIdx = Convert.ToInt32(Math.Floor(m_dblpValue[j - 1] * 10));

                    pLineColor = new Color();
                    pLineColor = Color.FromArgb(arrSepVerColors[intSepLineIdx, 0], arrSepVerColors[intSepLineIdx, 1], arrSepVerColors[intSepLineIdx, 2]);

                    AddVerticalLineSeries2(this, "ver_" + j.ToString(), pLineColor, m_cb[j], -3, m_intAreaChartHeight + 100);
        ...
        private double[] ClassSeparabilityMeasure(int intNClasses, int intNFeatures, IFeatureCursor pFCursor, int intEstIdx, int intVarIdx)
        {
            ...
            {
                frmProgress pfrmProgress = new frmProgress();
                if (intNFeatures > m_MaxFeaturesforPGB)
                {
                ...
                m_arrResults = new double[intNFeatures - 1];
                m_arrSortedProV = new double[intNFeatures - 1];
                ...
                for (int i = 0; i < intNFeatures - 1; i++)
                {
                    ...
                    for (int m = i; m >= 0; m--)
                    {
                        ...
                        {
                            dblCL = Math.Abs(m_arrEst[n] - m_arrEst[m]) / (Math.Sqrt(Math.Pow(m_arrVar[n], 2) + Math.Pow(m_arrVar[m], 2)));
                            if (dblCL < dblCLMin)
                                dblCLMin = dblCL;
                        }
                    }
                    m_arrResults[i] = dblCLMin;
                ...
                m_dblpValue = new double[intNClasses - 1];
                m_intResultIdx = new int[intNClasses - 1];
                for (int i = 0; i < intNClasses - 1; i++)
                {
                    m_intResultIdx[i] = System.Array.IndexOf(m_arrResults, arrSortedResult[i]);
                    //dblpValue[i] = pChart.DataManipulator.Statistics.NormalDistribution(arrSortedResult[i]);
                }
                System.Array.Sort(m_intResultIdx);

                Cs[0] = m_arrEst.Min();

                Cs[intNClasses] = m_arrEst.Max();
                for (int i = 0; i < intNClasses - 1; i++)
                {
                    Cs[i + 1] = m_arrEst[m_intResultIdx[i]];
                    m_dblpValue[i] = pChart.DataManipulator.Statistics.NormalDistribution(m_arrResults[m_intResultIdx[i]]);
                }

                //
                for (int i = 0; i < intNFeatures-1; i++)
                {
                    m_arrSortedProV[i] = pChart.DataManipulator.Statistics.NormalDistribution(arrSortedResult[i]);
                }
                pfrmProgress.Close();
                return Cs;
            }
        ...
        //                double dblMeanComp = Math.Pow(m_arrEst[i] - m_arrEst[j], 2) / (dblsquaredVar1 + dblsquaredVar2);
        ...
        //        NumericMatrix pBhattaDist = m_pEngine.CreateNumericMatrix(arrResults);
        //        m_pEngine.SetSymbol("Bhatta.diss", pBhattaDist);
        //        m_pEngine.Evaluate("library(cluster)");
        //        m_pEngine.Evaluate("kmed.result <- pam(as.dist(Bhatta.diss), diss=TRUE, " + intNClasses.ToString() + ")");
        //        NumericVector pClustering = m_pEngine.Evaluate("kmed.result$clustering").AsNumeric();
        ...
        //                if (pClustering[i] == j)
        //                {
        //                    double tempClassMax = m_arrEst[i];
        ...
        private void cboUncernFld_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                txtMinSep.Text = m_dblpValue.Min().ToString("N3");
                trbpValue.Value = Convert.ToInt32(m_dblpValue.Min() * 100);
            }
        }
        private void nudGCNClasses_ValueChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (m_intGCBreakeCount != intNewClassNumber)
                {
                    m_intGCBreakeCount = intNewClassNumber;
        ...
        //}
        private void AddUncertaintyRangeLineSeries(frmNewClassSeparbility pOptSample, string strSeriesName, System.Drawing.Color FillColor, double dblY, double dblXMin, double dblXMax)
        {
            try
            {
                var pSeries = new System.Windows.Forms.DataVisualization.Charting.Series
        ...
        private void AddVerEdgeLineSeries(frmNewClassSeparbility pOptSample, string strSeriesName, System.Drawing.Color FillColor, double dblY, double dblX, double dblScaleY)
        {
            try
            {
                var pSeries = new System.Windows.Forms.DataVisualization.Charting.Series
        ...
        public void AddClassResultsSeries(int intType)
        {
            ...
            if (m_cb == null)
                return;
            //if (intType == 1) //For Test
            //    cb = new double [7] { 34689, 41055, 43542, 45873, 48728, 53138, 67037 };
            //else if (intType == 2)
            //    cb = ChangeNBreaksGurobi(pModel, pTotalStepsConst, pDecVar, intGCBreakeCount, intNofFeatures);
            ...
            //this.pChart.ChartAreas[0].AxisX.LabelStyle.Format = "{0:#,##0.##}"; //Not Working
            ...
            this.pChart.ChartAreas[0].AxisX.LabelStyle.Angle = -90; // Change angle of Labels 
        ...
        private void AddVerticalLineSeries2(frmNewClassSeparbility pfrmOptSamp, string strSeriesName, System.Drawing.Color FillColor, double dblX, double dblYMin, double dblYMax)
        {
            try
            {
                var pSeries = new System.Windows.Forms.DataVisualization.Charting.Series
        ...
        private void AddAreaChartSeries(frmNewClassSeparbility pfrmOptSamp, string strSeriesName, System.Drawing.Color FillColor, double dblMinX, double dblMaxX, double dblY)
        {
            try
            {
                var pSeries = new System.Windows.Forms.DataVisualization.Charting.Series
        ...
        {
            if ( m_arrSortedProV == null)
                return -1;

            int intNewClassNumber = 0;
            while (m_arrSortedProV[intNewClassNumber] >= dblMinValue)
            {
                intNewClassNumber++;
            }
            intNewClassNumber += 1;
        ...
        private void frmNewClassSeparbility_Load(object sender, EventArgs e)
        {
            //Not Working
            //CultureInfo cultureInfo = new CultureInfo("en-US", false);
            //Thread.CurrentThread.CurrentCulture = cultureInfo;
            //pChart.ChartAreas[0].AxisY.LabelStyle.Format = "#,##0";
        }

        private void pChart_MouseUp_1(object sender, MouseEventArgs e)
        {
